...
void DFPlayerMini::completeCommand()
{
   while ( !commandCompleted() ) ;
}

bool DFPlayerMini::commandCompleted()
{
  
  if (_sending[Stack_ACK] && _sendingIndex == 0) {  //if the ack mode is on wait until the last transmition only if we are not in the middle of transmission
    if (_isSending) {
      delay(0);
      available();
    }
  ...
  if ( _sendingIndex == DFPLAYER_SEND_LENGTH) return true; 
  
  int buffer_cap = _serial->availableForWrite();
  if ( buffer_cap ) {
    for (int i=0; i<buffer_cap && _sendingIndex < DFPLAYER_SEND_LENGTH; i++) {
      _serial->write(_sending[ _sendingIndex++ ]);
    }
  ...
  return (_sendingIndex == DFPLAYER_SEND_LENGTH); 
}
...
void DFPlayerMini::sendStack(){
  ...
  Serial.println();
#endif
//  if (_serial.availableForWrite() >= DFPLAYER_SEND_LENGTH) { // might need to put 8 here for the FIFO length
